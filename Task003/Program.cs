...
void NewArray(double[] array)   //  метод для заполнения массива случайными числами double
{
  ...
  {
    array[i] = Convert.ToDouble(new Random().Next(100, 10000)) / 100;  // при делении на 100 мы получаем дробные значения. Функция "Convert.ToDouble" преобразует значения в тип double.
  }
}
